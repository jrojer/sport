...
struct Bst
{
    ...
    void UpdateHeight(node_t &a)
    {
        ...
        int left_size = a->left ? a->left->subtree_size : 0;
        int right_size = a->right ? a->right->subtree_size : 0;
        a->subtree_size = 1 + left_size + right_size;
    }
    ...
    int FindKMax(int k)
    {
        return FindKMax(k, root_);
    }
    int GetTreeSize(const node_t& root)
    {
        deque<node_t> q;
        q.push_back(root);
        int size = 0;
    ...
    int FindKMax(int k, node_t node)
    {
        int left_size = node->left ? node->left->subtree_size : 0;
        int right_size = node->right ? node->right->subtree_size : 0;
        //int correct_right_size = GetTreeSize(node->right);
        //int correct_left_size = GetTreeSize(node->left);
        if (k <= right_size)
        {
            return FindKMax(k, node->right);
        }
        else
        {
            k -= right_size;
